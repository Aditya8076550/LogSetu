...
export interface ULIPUniversalEvent {
  event_id: string;
  timestamp: string; // Event timestamp if present, or SOURCE_TIMESTAMP_UNAVAILABLE
  event_time?: string; // Preserved raw event time
  ingestion_time: string; // Exact system ingestion time
  ...
  ocsf_aligned: {
    class_uid: number; // 4001
    class_name: string; // "Network Activity"
    category_uid: number; // 4
...
export interface DiscoveredField {
  ...
  confidence: number; // 0 to 1 heuristic confidence
...
export interface ReplayValidationResult {
  ...
  required_fields_coverage: number; // 0.0 to 1.0
  ...
  sample_previews: Array<{
    raw: string;
    parsed: Partial<ULIPUniversalEvent>;
    valid: boolean;
  }>;
}

export interface ConfidenceSummary {
  ai_mapping_confidence: number; // percentage, e.g. 94%
  heuristic_label: string; // "Heuristic mapping confidence"
...
export interface ParserSpecification {
  source_identity: {
    ...
    vendor?: string;
    product?: string;
    format: 'delimited_kv' | 'delimited' | 'xml_tags' | 'json' | 'regex';
    fingerprint: {
      delimiter?: string;
      kv_delimiter?: string;
      header_delimiter?: string;
      ...
      segment_pattern?: number;
      min_delimiter_count?: number;
      action_tokens?: string[];
    };
  };
  field_mappings: Record<string, string>; // raw_field -> oCSF target (e.g. "U": "user.name")
  mappings: DiscoveredField[];
  event_mappings?: Record<string, string>; // source action -> standard action
  severity_mappings?: Record<string, string>; // source severity -> standard severity
